...
def preprocess_inputs(inputs): # noqa
    ...
    if isinstance(inputs, dict):
        ...
        if not ("sampling_rate" in inputs and ("raw" in inputs or "array" in inputs)):
            raise ValueError(
                "When passing a dictionary to ASRDiarizePipeline, the dict needs to contain a "  # noqa
                '"raw" key containing the numpy array representing the audio and a "sampling_rate" key, '  # noqa
...
def diarize_audio(
    diarizer_inputs: str, diarization_pipeline:dict[Any], num_speakers: int, 
    ...
    # diarizer output may contain consecutive segments from the same speaker 
    #   (e.g. {(0 -> 1, speaker_1), (1 -> 1.5, speaker_1), ...})
    # we combine these segments to give overall timestamps for each speaker's turn 
    #   (e.g. {(0 -> 1.5, speaker_1), ...})
    new_segments = []
...
def post_process_segments_and_transcripts(
    new_segments:dict[str], transcript:str, group_by_speaker:bool
) -> list:
    ...
    for segment in new_segments:
        ...
        # and cut the transcript to here
